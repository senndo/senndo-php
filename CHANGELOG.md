...
- `createVerification([...])`, `checkVerification([...])`, `getVerification($id)` : l’API Verify
  (`POST /v1/verify`, `POST /v1/verify/check`, `GET /v1/verify/{id}`). senndo génère, envoie par
  WhatsApp, expire et compte les essais ; le code n’est jamais rendu. Facturée à l’envoi, une fois
  par vérification ; le verdict de remise arrive avec le fournisseur (`delivery.status`), un
  destinataire sans WhatsApp échoue avec `recipient_not_on_whatsapp`. `createVerification` avec
  clé est retentée, sans clé jamais ; un contrôle consomme un essai et n’est jamais retenté.

## 1.2.0 — 2026-09-23

- `Webhook::verifySignature($secret, $enTete, $corps)` : authentifie un webhook reçu (`X-Senndo-Signature`), comparaison à durée constante et
...
* La règle « un envoi doit porter du contenu » accepte `content` au même titre que `text`,
  `media` et `template`.
* **PHP** : les dictionnaires libres (dont `content.variables`) sont typés `array<array-key, mixed>`. PHP convertit la clé `'1'` en entier ; l'ancien type `array<string, mixed>` faisait refuser par PHPStan un appel pourtant correct.
...
```php
$approuve = $pays['country'] === 'FRA' && $pays['status'] === 'approved';
```
...
`CountryIso3` et `CountryAlpha2` sont déclarés en `@phpstan-type` et portés par les trois champs `country` du
...
### Note — ce qui peut ne plus passer PHPStan chez vous

Ces deux corrections ne retirent rien à la réponse, mais elles **resserrent des types**, et un
type plus étroit peut faire échouer une analyse qui passait :

- une correspondance exhaustive sur `MessageStatus` doit maintenant traiter `unknown` ;
- un `isset($row['senderId'])` employé pour tester l'absence décrit une branche morte : la clé
  était déjà toujours là — et `isset` reste `false` sur une valeur `null`, ce qui n'a jamais
  signifié « clé absente ».

`0.1.2` restera disponible sur Packagist ; nous ne dépublions rien.
